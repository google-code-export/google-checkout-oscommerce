...
class gcXmlBuilder {
  ...
  // Used when an element has sub-elements, adds an open tag to the output.
  ...
  // Data within the open and close tags are provided with $contents.
  function Element($element, $contents, $attributes = array()) {
    $this->_indent();
    $this->xml .= '<'. $element;
    foreach ($attributes as $key => $value) {
      $this->xml .= ' '. $key .'="'. htmlentities($value) .'"';
    }
    $this->xml .= '>'. htmlentities($contents) .'</'. $element .'>'."\n";
  }
