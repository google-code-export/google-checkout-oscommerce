    ...
    switch ($action) {
      case 'update_order':
        ...
        $check_status = tep_db_fetch_array($check_status_query);
				...
				if($check_status['orders_status'] == '1' &&  $status == '2') {
				  chdir('./..');
					$curr_dir = getcwd();
          ...
          $google_order = $google_answer['google_order_number'];  
		        ...
		        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
						...
						fwrite($message_log, sprintf("\r\n%s\n",$postargs));
		        send_req($googlepay->request_url, $googlepay->merchantid, $googlepay->merchantkey, 
				...
				if($check_status['orders_status'] == '2' &&  $status == '3') {
				  chdir('./..');
					$curr_dir = getcwd();
         	define('API_CALLBACK_MESSAGE_LOG', $curr_dir."/googlecheckout/response_message.log");
       	  define('API_CALLBACK_ERROR_LOG', $curr_dir."/googlecheckout/response_error.log");
          ...
          if (!$message_log = fopen(API_CALLBACK_MESSAGE_LOG, "a")) {
            error_func("Cannot open " . API_CALLBACK_MESSAGE_LOG . " file.\n", 0);
		        exit(1);
					...
					$google_answer = tep_db_fetch_array(tep_db_query("select google_order_number from " . $googlepay->table_order . " where orders_id = " . (int)$oID ));
          $google_order = $google_answer['google_order_number'];  
					if($google_order != '') {					
		        $postargs = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>
