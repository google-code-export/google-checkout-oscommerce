...
// for the check boxes for the shipping options   				
      if($module->code = "googlechecout" && strpos($key, ";") != FALSE) {
        $configure_key= substr($key, 0, strpos($key, ";"));
        if($value != "0") {
          $checkout .= " ".$value;
		    }
        continue;
...
// ** END GOOGLE CHECKOUT **
      tep_db_query("update " . TABLE_CONFIGURATION . " set configuration_value = '" . $value . "' where configuration_key = '" . $key . "'");
		...
		if($configure_key != '') {
		  tep_db_query("update " . TABLE_CONFIGURATION . " set configuration_value = '" . $checkout . "' where configuration_key = '" . $configure_key . "'");	
		}
