...
// Added to process check boxes in the admin UI
  function tep_cfg_multi_select_option($select_array, $key_value, $key = '') {
    $string = '';
    $options = array();
    ...
    for ($i=0, $n=sizeof($select_array); $i<$n; $i++) {
      $name = ((tep_not_null($key)) ? 'configuration[' . $key .';'.$select_array[$i]. ']': 'configuration_value'.';'.$select_array[$i]);
